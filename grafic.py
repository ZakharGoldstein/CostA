...
def main(page):
    ...
    def add_clicked(e):
        if new_task.value and new_count.value.isdecimal() and category_dropdown.value:
            task_name = new_task.value.lower()
            task_count = int(new_count.value)
            tasks[task_name] = task_count  # Добавляем задачу в словарь
            Backend.new_purchase(task_count, task_name, category_dropdown.value)  # Передаем категорию в функцию
    ...
    new_task = ft.TextField(hint_text="Покупка", width=300)
    ...
    page.add(ft.Row([new_task, category_dropdown, new_count, ft.ElevatedButton("Добавить", on_click=add_clicked)]))
